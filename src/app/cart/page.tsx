...
export default function CartPage() {
  ...
  const handleApplyPromo = async () => {
    ...
    try {
      ...
      addToast('ОШИБКА ПРОВЕРКИ ПРОМОКОДА', 'error');
    }
  };

  // 2. РАСЧЕТ ЦЕНЫ (Один раз, без дублей)
  ...
  const handleCheckout = async () => {
    ...
    // Замораживаем значения для оплаты
    ...
    try {
      ...
      // Формируем товары (используем any чтобы не было конфликтов типов)
      ...
      if (orderError) {
        addToast(`ОШИБКА ЗАКАЗА: ${orderError.message}`, 'error');
        setIsOrdering(false);
        return;
      }
      ...
      addToast('ПРОИЗОШЛА КРИТИЧЕСКАЯ ОШИБКА', 'error');
      console.error(e);
    } finally {
      setIsOrdering(false);
    }
